...
options:
  ...
  --version  Print version\n`
      let actual = String(execSync('./bin/uselesscss --help'))
  ...
  describe('Positional args', function () {
    ...
    it('Should return nothing if no <css> is provided', function () {
      ...
      let actual = String(execSync(`./bin/uselesscss ${tempHtmlFile.path}`))
    ...
    it('Should read <css> from STDIN, negative case', function () {
      ...
      let actual = String(execSync(`./bin/uselesscss ${tempHtmlFile.path} - <${tempCssFile.path}`))
      assert.strictEqual('', actual)
    })
